...
>
> `r tufte::quote_footer('Yes or No?')`
...
```{r}
survey_responses <- read.csv("https://raw.githubusercontent.com/keithmcnulty/peopleanalytics-regression-book/master/presentations/survey_responses.csv")
...
> "Do you think sentiment or intention to leave will make a difference in departures over the next two years?"
>
> `r tufte::quote_footer('None, Sentiment Only, Intention Only, Both')`
...
```{r}
departure_dates <- read.csv("https://raw.githubusercontent.com/keithmcnulty/peopleanalytics-regression-book/master/presentations/departure_dates.csv")
...
## Creating the survivial outcome
...
colnames(survey) <- c("iid", "role", "delete3", "roletitle", "delete5", "delete6",
                      ...
                      "tech_other_vendor",
                      "tech_python", "tech_r", "tech_spss", "tech_sql", "delete50", "delete51")
...
model <- polr("deliver ~ know_ai + know_cloud + know_cleaning + know_engineering + know_reporting +
                      know_explanatory + know_eda + know_ona + know_predictive", data = survey)
...
effectsizes_skilled <- data.frame(matrix(unlist(effectsizes_skilled), nrow=length(effectsizes_skilled), byrow=TRUE)) %>% 
  ...
  dplyr::mutate(Effect = dplyr::case_when(
    abs(`Cohen's d`) < 0.15 ~ "Very Small/Negligible",
    between(abs(`Cohen's d`), 0.15, 0.3) ~ "Small",
    between(abs(`Cohen's d`), 0.300001, 0.6) ~ "Moderate",
    abs(`Cohen's d`) > 0.6 ~ "Large",
    TRUE ~ "Unknown"
  ))
```
