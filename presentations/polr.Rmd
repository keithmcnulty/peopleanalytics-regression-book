...
In a linear model we would use an $R^2$ value to get a good point of view on this.  In a logistic model there are several variants of this called *pseudo-* $R^2$.  Let's look at a few popular variants.
...
This statistic should be referenced with care.  For example:
...
The quickest way is to do a Brant-Wald test on your model, looking for p-values less than 0.05 to indicate failure of the test.
