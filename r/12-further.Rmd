...
2.  Transform any categorical data you intend to use as an input variable into dummy variables.
3.  Perform any other transformations on the data to help with the interpretation of model results.  For example, are the numerical scales intuitive for interpretation?
...
The `learning` data set contains `r nrow(learning)` instances of feedback from `r length(unique(learning$idcode))` different participants in a range of learning programs offered by an executive education provider.  Each row of data represents the feedback of a specific participant on a specific program that they participated in.  Participants where not required to respond to all feedback questions and any question where no response was given is indicated as `NA`.
