...
The data set can be found [here](http://peopleanalytics-regression-book.org/data/health_insurance.csv) and consists of the following fields:
...
We see that all variables except tenure seem to play a significant role in the choice of Product A.  All else being equal, being older makes the choice of Product A less likely. Males are more likely to choose Product A, and more children and higher levels of seniority also make the choice of Product A more likely.  Based on this, we can consider simplifying our model to remove `tenure`.  We can also calculate odds ratios and perform some model diagnostics if we wish, similar to how we approached the problem in the previous chapter.
...
```{r}
A_simple <- glm(formula = productA ~ age + children + gender + position_level, 
...
In a similar way we can produce two other models, representing the choice of Products B and C.  These models produce similar significant variables, except that `position_level` does not appear to be significant in the choice of Product C.  If we simplify all our three models we will have a slightly differently defined models for the choice of Product C versus our models for the other two product choices.  However, we can conclude in general that the only input variable that seems to be non-significant across all choices of product is `tenure`.
...
In our case, we can compare the coefficients of the model with and without `tenure` included and verify that the changes in the coefficients are not substantial.
