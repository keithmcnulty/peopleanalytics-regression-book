...
```{r prop-odds-int, fig.cap="Proportional odds model illustration for a 5-point Likert survey scale outcome greater than 3 on a single input variable.  Each cutoff point in the latent continuous outcome variable $y'$ gives rise to a binomial logistic function.", fig.align = "center", echo = FALSE, out.width = if (knitr::is_latex_output()) {"90%"}}
library(ggplot2)
...
The `MASS` package provides a function `polr()` for running a proportional odds logistic regression on a dataset in a similar way to our previous models.  The key (and obvious) requirement is that the outcome is an ordered factor.  Since we did our conversions in Section \@ref(ord-walkthrough) we are ready to run this model, and we will start by running it on all input variables, and let the `polr()` function handle our dummy variables automatically.
...
We can display all our critical statistics by combining a few of these statistics into a dataframe.
