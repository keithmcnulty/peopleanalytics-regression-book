...
* *Continuation-ratio* logistic model.  This model compares each level of the ordinal variable to all lower levels.  This can be modeled using binary logistic regression techniques, but new variables need to be constructed from the dataset to allow this.  `r if (knitr::is_latex_output()) '\\index{R!packages!rms@\\texttt{rms}}'`The R package `rms` has a function `cr.setup()` which is a utility for preparing an outcome variable for a continuation ratio model.
`r if (knitr::is_latex_output()) '\\index{proportional odds logistic regression|)}'`
