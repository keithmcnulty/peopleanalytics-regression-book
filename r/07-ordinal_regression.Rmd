# Ordinal Logistic Regression for Ordered Category Outcomes
...
```{r}
url <- "https://raw.githubusercontent.com/keithmcnulty/eampa/master/data/soccer.csv"
soccer <- read.csv(url)
...
```{r}
# convert discipline, level and result to ordered factor
soccer$discipline <- ordered(soccer$discipline, levels = c("None", "Yellow", "Red"))
...
```{r}
# apply as.factor to two columns
...
* Each additional red card received in the prior 25 games is associated with an approximately 47% higher odds of greater diciplinary action by the referee
...
DescTools::PseudoR2(model, which = c("McFadden", "CoxSnell", "Nagelkerke", "AIC"))
```

There are numerous tests of goodness of fit/model confidence that can apply to ordinal logistic regression models, and this area is the subject of considerable recent research.  The `generalhoslem` package in R contains routes to four possible tests, with two of them particularly recommended for ordinal models.  Each work in a similar way to the Hosmer-Lemeshow test discussed in \@ref(logistic-gof), by dividing the sample into groups and comparing the observed versus the fitted outcomes using a chi-square test.  Since the null hypothesis is a good model fit, low p-values indicate potential problems with the model.   We run these tests below for reference. For more information, see @fagerland-hosmer, and for a really intensive treatment of ordinal data modeling @agresti-ordinal is recommended. 
...
# pulkstenis-robinson test (requires the vector of categorical input variables as an argument)
...
The Wald test is conducted on the comparison of the proportional odds and generalized models. A Wald test is a hypothesis test of the significance of the difference in model coefficients, producing a chi-square statistic.  A low p-value in a Brant-Wald test is an indicator that the coefficient does not satisfy the proportional odds assumption.  The `brant` package in R provides an implementation of the Brant test, and in this case supports our judgment that the proportional odds assumption holds.
...
In the event where the option to remove variables is unattractive, alternative models for ordinal outcomes should be considered.  The most common alternatives (which we will not cover in depth here, but are explored in @agresti-ordinal) are:
...
5.  Describe some approaches for assessing the fit and confidence of an ordinal logistic regression model.
...
The `managers` data set can be found at https://raw.githubusercontent.com/keithmcnulty/eampa/master/data/managers.csv.  It is a set of information of 571 managers in a sales organization, and consists of the following fields:
