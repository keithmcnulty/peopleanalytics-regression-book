...
As an example to illustrate the mathematical relationship between these criteria, let's assume that we run an experiment on a group of employees of size $n$ where we introduce a new benefit and then test their satisfaction levels before and after its introduction.  As a statistic of a random variable, we can expect the mean difference in satisfaction to have a normal distribution.  Therefore we define the effect size in this case as the difference in the before and after means as a proportion of the combined standard deviation - known as Cohen's $d$:

$$
d = \frac{\mu_1 - \mu_0}{\sigma}
$$

where $\mu_0$ and $\mu_1$ are the means of the before and after samples respectively and $\sigma$ is the standard deviation of the combined samples. Recall from Chapter \@ref(found-stats) that to meet a statistical significance standard of $\alpha$, we will need $d$ to be greater than a certain multiple of the standard error $\frac{\sigma}{\sqrt{n}}$.  Let's call that multiple $z_{\alpha}$.   Therefore, we can say that the statistical power of our hypothesis test is:

$$
P(\frac{\mu_1 - \mu_0}{\sigma} > \mu_0 + z_{\alpha}\frac{\sigma}{\sqrt{n}})
$$
Therefore, we can see that to calculate power depends on a measure of the observed difference between our two samples (through knowing $\mu_0$, $\mu_1$ and $\sigma$), the significance level $\alpha$ and the sample size $n$^[We will also need to know the expected distribution of the statistics that we are analyzing in order to determine the power probability].
