...
knitr::include_graphics("~/rstudio_projects/eampa/www/01/Regression-to-the-mean.png")
```
...
```{r}
interaction_model <- lm(data = ugtests, formula = Final ~ Yr2 + Yr2*Yr3)
summary(interaction_model)
```

We see that introducing this interaction term has improved our model, and that the interaction term is significant, so we conclude that in addition to a significant effect of the `Yr2` an `Yr3` scores, there is an additional significant effect from their interaction.  Visualizing this, we can see in Interactive Figure \@ref(fig:non-lin-reg-3d) that this non-linear term introduces a curvature to our fitted surface that aligns it more closely with the observations in our data set. 
