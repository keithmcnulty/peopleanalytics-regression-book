...
Figure \@ref(fig:galton-fig) is an illustration of Galton's data with the black solid line showing what a perfect relationship would look like, and the red dashed line showing the actual relationship he determined^[This representation of Galton's study originates from @senn].  You can regard the red dashed line as 'going back' from the perfect relationship (symbolized by the black line) in the direction of a horizontal line representing the mean of the child heights.  This might give you an intuition that will help you understand later sections of this chapter. In an arbitrary data set, the red dashed line can lie anywhere between a horizontal line at the mean of $y$ (no relationship) and the solid black line (a perfect relationship). Linear Regression is about finding the red dashed line in your data and using it to explain the degree to which your input data (the $x$ axis) explains your outcome data (the $y$ axis). 
...
Now let's assume that we have a sample of ten observations with which to estimate our linear relationship.  Let's take the first ten values of `Yr3` and `Final` in our `ugtests` data set:

```{r}
(d <- head(ugtests[ , c("Yr3", "Final")], 10))
```

We can do a simple plot of these data points as in Figure \@ref(fig:sample-plot). Intuitively, we can imagine a line passing through these points that 'fits' the general pattern.  For example, taking $m = 1.2$ and $c = 5$, the resulting line $y = 1.2x + 5$ could fit between the points we are given, as displayed in Figure \@ref(fig:sample-plot-fit).

```{r sample-plot, fig.cap = "Basic scatter plot of ten datapoints", fig.align = "center", echo = FALSE, out.width = if (knitr::is_latex_output()) {"90%"}}
ggplot2::ggplot(data = d, aes(x = Yr3, y = Final)) +
  ggplot2::geom_point()
```


``` {r sample-plot-fit, fig.cap = "Fitting $y=1.2x + 5$ to our ten datapoints", fig.align = "center", echo = FALSE, out.width = if (knitr::is_latex_output()) {"90%"}}
ggplot2::ggplot(data = d, aes(x = Yr3, y = Final)) + 
  ggplot2::geom_point() +
...
model$coefficients
```
`r if (knitr::is_latex_output()) '\\index{linear regression!fitting}'`This tells us that that our best fit model---the one that minimizes the sum of the squares of the residuals---is $y = 1.14x + 16.63$.  In other words, our `Final` test score can be expected to take a value of 16.63 even with zero score in the `Yr3` input, and that every additional point scored in `Yr3`  will increase the `Final` score by 1.14.
...
Recall from Section \@ref(mean-var-sd) the definition of the population variance of $y$, notated as $\mathrm{Var}(y)$.  Note that it is defined as the average of the squares of the residuals around the mean of $y$. Therefore $\mathrm{Var}(y)$ represents the mean sum of squares error of a random model.  This calculates in this case to `r round((9/10)*var(d$Final), 2)`.  Let's overlay our fitted model onto this random model in Figure \@ref(fig:model-overlay).
...
So for most of our observations (though not all) we seem to have reduced the 'distance' from the random model by fitting our new model.  If we average the square of our residuals for the fitted model, we obtain the mean sum of squares error of our fitted model, which calculates to `r round(sum(model$residuals^2)/10, 2)`.

Therefore before we fit our model, we have an average error of `r round((9/10)*var(d$Final), 2)`, and after we fit it, we have an average error of `r round(sum(model$residuals^2)/10, 2)`.  So we have reduced the average error of our model by `r round((9/10)*var(d$Final), 2) - round(sum(model$residuals^2)/10, 2)` or, expressed as a proportion, by `r round((round((9/10)*var(d$Final), 2) - round(sum(model$residuals^2)/10, 2))/((9/10)*var(d$Final)), 2)`.  In other words, we can say that our model explains `r  round(((9/10)*var(d$Final) - sum(model$residuals^2)/10)/((9/10)*var(d$Final)), 2)` (or `r  round(((9/10)*var(d$Final) - sum(model$residuals^2)/10)/((9/10)*var(d$Final)), 2) * 100`%) of the variance of our outcome.  

`r if (knitr::is_latex_output()) '\\index{linear regression!R-squared}'`This metric is known as the $R^2$ of our model and is the primary metric used in measuring the fit of a linear regression model^[As a side note, in a simple regression model like this, where there is only one input variable, we have the simple identity $R^2 = r^2$, where $r$ is the correlation between the input and outcome (for our small set of ten observations here, the correlation is `r round(cor(d$Yr3, d$Final), 3)`)]. 


## Multiple linear regression {#multiple-linear-regression}

`r if (knitr::is_latex_output()) '\\index{linear regression!multiple linear regression}'`In reality, regression problems rarely involve one single input variable, but rather multiple variables. The methodology for multiple linear regression is similar in nature to a simple linear regression, but obviously more difficult to visualize because of its increased dimensionality.

In this case, our input is a set of $p$ variables $x_1, x_2, \dots, x_p$.  Extending the linear equation in Section \@ref(linear-single), we seek to develop an equation of the form:
...
`r if (knitr::is_latex_output()) '\\index{linear regression!goodness-of-fit}'`This provides us with some of the most important metrics from our model.  In particular, the last line gives us a report on our overall model confidence or 'goodness-of-fit'---that is, how confident can we be that our model fits the outcome better than the alternative of a random model.  `r if (knitr::is_latex_output()) '\\index{linear regression!F-statistic}'`The F-statistic is an evaluation of the confidence of the entire model, with a higher F-statistic indicating a strong likelihood that the model fits the data better than a random model.  More intuitively, perhaps, we also have the p-value for the F-statistic.  In this case it is extremely small, so we can conclude that our model has some explanatory power over and above a random model.
...
* For cars of the same make, each *extra dollar* spent on manufacturing changes the retail price by ...
* For cars of the same manufacturing cost, if the vehicle is an Audi, the retail price changes by ...
* For cars of the same manufacturing cost, if the vehicle is a Ford, the retail price changes by ...
* For cars of the same manufacturing cost, if the vehicle is a Toyota, the retail price changes by ...
...
```{r non-lin-reg-3d, fig.cap = if (knitr::is_latex_output()) {"3D Visualization of the fitted \\texttt{interaction\\_model} against the \\texttt{ugtests} data"} else {"3D Visualization of the fitted `interaction_model` against the `ugtests` data"}, fig.align="center", echo = FALSE, out.width = if (knitr::is_latex_output()) {"90%"}}
