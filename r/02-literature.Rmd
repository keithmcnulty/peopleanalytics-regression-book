...
AIC(full_model)
```
We can see that the model which contains only our two significant inputs - sales and customer rating - is determined to be the most parsimonious model according to the AIC. Note that the AIC should not be used to interpret model quality or confidence - it is possible that the lowest AIC might be telling you which model is the best of a bad bunch.
...
5. Experiment with model parsimony by reducing input variables that do not have a significant impact on the outcome.  Decide on the most parsimonious model.
6. Calculate a variety of Pseudo-$R^2$ variants for the model.  How would you explain these to someone?
7. Report the conclusions of your modeling exercise to the charity using a simple explanation that assumes no knowledge of statistics.
8. Extension:  Using a variety of methods of your choice, test the hypothesis that your model fits the data.  How conclusive are your tests?
