...
Modeling an outcome with the primary goal of understanding what influences it can be quite a different matter to modeling an outcome with the primary goal of predicting if it will happen in the future.  If we need to understand what influences an outcome, we need to get inside a model and construct a formula or structure to infer how each variable acts on that outcome, we need to get a sense of which variables are meaningful or not, and we need to examine how much of the variability in the outcome can be explained by our variables. If our primary aim is to predict the outcome, getting inside the model is less important because we don't have to explain the outcome, we just need to be confident that it predicts accurately.  

A model constructed to understand an outcome is often called an *inferential* model.  Regression models are the most well-known and well-used inferential models available, providing a wide range of measures and insights that help us explain the relationship between our input variables and our outcome of interest, as we will see in future chapters of this book.

The current reality in the field of people analytics is that inferential models are more required than predictive models.  There are many reasons for this.  Firstly, datasets in people analytics are rarely large enough to facilitate satisfactory prediction accuracy, and so attention is usually shifted to inference for this reason alone.  Secondly, even in the rare situations where accurate predictive modeling is attainable, stakeholders are unlikely to trust the output of predictive models without some sort of elementary understanding of how the predictions are generated, and this requires the analyst to consider inference power as well as predictive accuracy in their modeling approach.  Again, regression models come to the fore because they are commonly able to provide both inferential and predictive power. 
...
Note that there is a difference between establishing a statistical relationship between $\mathscr{C}$ and $\mathscr{O}$ and establishing a *causal relationship* between the two.  This can be a common trap that inexperienced statistical analysts fall into when communicating the conclusions of their modeling.  Establishing that a relationship exist between a construct and an outcome is a far cry from being able to say that one *causes* the other.  This is the common truism that 'correlation does not equal causation'.
...
5.  Proposing a set of models to test for $f$
...
* Chapter 2 covers the basics of the R programming language for those who want to attempt to jump straight in to the work in subsequent chapters but have no R experience  
* Chapter 3 covers the essential statistical concepts needed for subsequent chapters, illustrated with real data
...
* Chapter 6 covers multinomial regression.  The walkthrough example and exercise involves modeling the choice of health insurance policy by company employees based on demographic and position data.
* Chapter 7 covers ordinal regression.  The walkthrough example involves modeling in-game disciplinary action against soccer players based o n prior discipline and other factors.  The exercises involve predicting manager performance based on varied data.
* Chapter 8 covers modeling options for data with explicit or latent hierarchy.  The first part covers mixed modeling and uses a model of speed dating decisions as a walkthrough and example.  The second part covers latent variable modeling and uses an employee engagement survey as a walkthrough and exercise.
* Chaoter 9 covers survival analysis and cox proportional hazards regression and uses employee attrition as a walkthrough example and exercise.
