...
The **mean** is the average value of the data and is defined by adding up all the values and dividing by the number of observations.  If the data set $x$ contains the $n$ observations $x_1, x_2, ..., x_n$, then the mean $\bar{x}$ is defined as:
...
The **variance** is a measure of how much the data varies around its mean.  There are two different definitions of variance.  The **population variance** assumes that that we are working with the entire population and is defined as the average squared difference from the mean.  If $\bar{x}$ is the mean value of $x = x_1, x_2, ..., x_n$, then the population variance is
...
You can see that, on the same data set,
...
The *covariance* between two variables is a measure of the extent to which one changes as the other changes.  If $y = y_1, y_2, ..., y_n$ is a second variable, and $\bar{x}$ and $\bar{y}$ are the means of $x$ and $y$ respectively, then the **sample covariance** of $x$ and $y$ is defined as:
...
To calculate how many standard errors we would need around the sample mean to have a 95% probability of including the true population mean, we need to use the $t$-distribution.  The $t$-distribution is essentially an approximation of the normal distribution acknowledging that we only have a sample estimate of the true population standard deviation in how we calculate the standard error.  In this case where we are dealing with a sample mean, we use the $t$-distribution with $n-1$ degrees of freedom. We can use the `qt()` function in R to find the standard error multiple associated with the level of certainty we need.  This is called the *$t$-statistic*.  In this case we are looking for our true population mean to be outside the top 2.5% or bottom 2.5% of the distribution.
...
Observations about the distribution of statistics on random variables allow us to construct tests for hypotheses of statistical difference or similarity.  Such hypothesis testing is useful in itself for simple bivariate analysis in practice settings, but it will be particularly critical in later chapters in determining whether models are useful or not.  Before we go through some technical examples of hypothesis testing, let's overview the logic and intuition for how hypothesis testing works.

The purpose of hypothesis testing is to establish a high degree of statistical certainty regarding a claim of statistical difference in a population based on the properties of a sample.  Consistent with a high statistical burden of proof, we start from the hypothesis that there is no statistical difference, called the *null hypothesis*.  We do not reject the null hypothesis unless the statistical properties of the data render the null hypothesis very unlikely.  If this is indeed the case, we reject the null hypothesis in favour of the *alternative hypothesis* that a statistical difference does exist in the population.  

Most hypothesis tests can return a *p-value*, which effectively translates to a probability that the null hypothesis is true for our population.  There are some common standards to the level of p-value needed to reject a null hypothesis.  The most common standard is a p-value less than 0.05.  This means we are comfortable with the possibility that 1 in 20 times we will be wrong and we will conclude that there is a difference when in fact there is not.  As you can imagine, there will be many situations where we need an even greater level of certainty.  In pharmaceutical trials, for example, we might need a 1 in 10,000 or 1 in 100,000 chance that we are wrong when we conclude that a drug is safe to take. 

Three common such hypothesis tests are:

1. Testing for a difference in the means of two groups in a population
2. Testing for a non-zero correlation between two variables
3. Testing for a difference in frequency distributions between different categories in a population.  

We will go through an example of each of these.  During these examples we will go through the steps of manually calculating all the statistics needed to do the hypothesis testing, before we demonstrate the functions that perform the full test for you.  Users don't absolutely need to know all the details contained in this section, but a strong understanding of the underlying methods are encouraged.
...
$$
where $\sigma_{\mathrm{perf1}}$ and $\sigma_{\mathrm{perf2}}$ are the standard deviations of the two samples and $n_{\mathrm{perf1}}$ and $n_{\mathrm{perf2}}$ are the two sample sizes.  

```{r, include = FALSE}
# calculate welch-satterthwaite estimate of degrees of freedom
nx <- length(perf1$sales)
ny <- length(perf4$sales)
stderrx <- sqrt(var(perf1$sales)/nx)
stderry <- sqrt(var(perf4$sales)/ny)

stderr <- sqrt(stderrx^2 + stderry^2)
df <- stderr^4/(stderrx^4/(nx-1) + stderry^4/(ny-1))
```

We use a special formula called the [Welch-Satterthwaite approximation](https://en.wikipedia.org/wiki/Welch%E2%80%93Satterthwaite_equation) to calculate the degrees of freedom for the two samples, which in this case calculates to `r round(df, 2)`^[I've kept the gory details of how this is derived out of view, but you can see them if you view the source code for this book].  This allows us to construct a 95% confidence interval for the difference between the means, and we can test whether this contains zero.
...
```{r norm-dist, fig.cap = "t-distribution of the mean sales difference between `perf1` and `perf4`, 95% confidence intervals (red lines) and a zero difference (blue line)", fig.align = "center", echo = FALSE}
...
This confirms an extremely low p-value.  Nowadays, it is never necessary to do these manual calculations ourselves because hypothesis tests are a standard part of statistical software.  In R, the `t.test()` function performs a hypothesis test of no mean difference on two samples and confirms our manually calculated p-value and 95% confidence interval.
