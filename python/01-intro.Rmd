...
We can see from the R-squared value that our model explains a lot of the variance in the Final examination score.   Alternatively, we can view the entire summary to receive a formatted report on our model. We can also see a report on our model confidence - that is, how confident can we be that our model explains the outcome to a non-zero degree.  The F-statistic is an evaluation of the confidence of the entire model, with a higher F-statistic indicating a better model.  More intuitively, perhaps, we also have the p-value for the model, notated as `Prob (F-statistic)`.  In this case it is extremely small, so we can conclude (unsurprisingly) that our model has some explanatory/predictive power.
