...
If your outcome of interest is on a binary or categorical scale (for example, 'Yes' or 'No'; 'High', 'Medium' or 'Low'), then linear regression is not recommended and logistic regression or other probabilistic methods are more well-suited, and we will cover these in subsequent chapters.

### Walkthrough Example {#walkthrough-ols}

We will use walkthrough examples throughout most chapters of this book to maintain a focus on a real problem as we go through the essential theory and methodology, and to demonstrate the coding of the approach in Python.  The data sets for all walkthrough examples will be downloadable via the link provided.  
...
To help with this, you have been provided with data for last year's individuals graduating from the degree program, and you have been asked to create a model to explain each individual's final examination score based on their examination scores for the first three years of their program.
...
The data looks as expected, with test scores for four years all read in as numeric data types, but of course this is only a few rows.  We need a quick statistical overview of the data.
...
We can see that the tests have different scales in Years 3 and 4, and judging by the means and upper quartiles, students seem to have found earlier exams more challenging.  We can also be assured that there is no missing data, as these would have been displayed as `NaN` counts if they existed.

We can also plot our four years of test scores pairwise to see any initial relationships of interest.

```{python pairplot-ugtests, fig.cap = "Pairplot of the `ugtests` data set", fig.align = "center"}
# import matplotlib if necessary
# import matplotlib.pyplot as plt

import seaborn as sns

# display a pair plot of all four columns of data
pairplot = sns.pairplot(ugtests)
plt.show(pairplot)

```

In the diagonal we can see the distributions of the data in each column.  We observe a relatively normal looking distribution of our outcome data (`Final`), with more skewed distributions of data for previous years.  We can see scatter plots off the diagonal to diagnose possible correlations.  For example, we see a particularly strong correlation between `Yr3` and `Final` test scores, along with some other notable correlations elsewhere.


## Simple Linear Regression {#simple-ols}

In order to visualize our approach and improve our intuition, we will start with *simple* linear regression, which is the case where there is only a single input variable and output variable.
...
$$y = mx + c$$
where $m$ represents the slope or gradient of the line, and $c$ represents the point at which the line intercepts the $y$ axis.  When using a straight line to model a relationship in the data, we call $c$ and $m$ the *coefficients* or *parameters* of the model.

Now let's assume that we have a sample of six datapoints with which to estimate our linear relationship.  Let's take the first six values of `Yr3` and `Final` in our `ugtests` dataset:

```{python}
ugtests.head(6)[["Yr3", "Final"]]
```

We can do a simple plot of these data points as in Figure \@ref(fig:sample-plot):

```{r sample-plot, fig.cap = "Basic scatter plot of six datapoints", fig.align = "center", echo = FALSE}
d <- head(py$ugtests[ , c("Yr3", "Final")])
ggplot2::ggplot(data = d, aes(x = Yr3, y = Final)) +
  ggplot2::geom_point()
```

Intuitively, we can imagine a line passing through these points that 'fits' the general pattern.  For example, taking $m = 2$ and $c = 0$, the resulting line $y = 2x$ could  fit between the points we are given, as displayed in Figure \@ref(fig:sample-plot-fit):

``` {r sample-plot-fit, fig.cap = "Fitting $y=2x$ to our six datapoints", fig.align = "center", echo = FALSE}
ggplot2::ggplot(data = d, aes(x = Yr3, y = Final)) + 
  ggplot2::geom_point() +
  ggplot2::geom_function(fun = function(x) 2*x, colour = "red", linetype = "dashed") +
  ggplot2::annotate("text", x = 165, y = 300, label = TeX("$y = 2x$"), colour = "red")
...
``` {r sample-plot-residuals, fig.cap = "Residuals of $y=2x$ for our six datapoints", fig.align = "center", echo = FALSE}
ggplot2::ggplot(data = d, aes(x = Yr3, y = Final)) + 
  ggplot2::geom_point() +
  ggplot2::geom_function(fun = function(x) 2*x, colour = "red", linetype = "dashed") +
  ggplot2::annotate("text", x = 165, y = 300, label = TeX("$y = 2x$"), colour = "red") +
  ggplot2::geom_segment(aes(x = Yr3[1], y = 2*Yr3[1], xend = Yr3[1], yend = Final[1]), color = "red") +
  ggplot2::geom_segment(aes(x = Yr3[2], y = 2*Yr3[2], xend = Yr3[2], yend = Final[2]), color = "red") +
  ggplot2::geom_segment(aes(x = Yr3[3], y = 2*Yr3[3], xend = Yr3[3], yend = Final[3]), color = "red") +
  ggplot2::geom_segment(aes(x = Yr3[4], y = 2*Yr3[4], xend = Yr3[4], yend = Final[4]), color = "red") +
  ggplot2::geom_segment(aes(x = Yr3[5], y = 2*Yr3[5], xend = Yr3[5], yend = Final[5]), color = "red") +
  ggplot2::geom_segment(aes(x = Yr3[6], y = 2*Yr3[6], xend = Yr3[6], yend = Final[6]), color = "red")
```
...
For a number of reasons (not least that fact that at the time this method was developed it was one of the easiest to calculate manually), the most common approach is number 3, which is why we call our regression model *Ordinary Least Squares* regression.  Some simple algebra and calculus can help us determine the equation of the line that generates the least squared residual error.  You can see more of the mathematics [here](https://are.berkeley.edu/courses/EEP118/current/derive_ols.pdf), but let's look at how this works in practice.  
...
First we need to express the model we are looking to calculate as a formula.  In this simple case we want to regress the outcome $y =$ `Final` against the input $x =$ `Yr3`, and therefore we would use the simple formula notation `Final ~ Yr3`.  Now we can use the `ols()` function from the `statsmodels` package function to define the OLS model based on our dataset and our formula, and then fit the model.

```{python}
import statsmodels.formula.api as sm

d = ugtests.head(6)[["Yr3", "Final"]] 

## create and fit model
model = sm.ols(formula = "Final ~ Yr3", data = d)
results = model.fit()

```
The `results` object that we have created is contains numerous attributes related to the fitted model (you can see the names of all of them by calling `dir(results)`).  However, we are interested in the `params` attribute which will tell us the coefficients of our model:

```{python}
print(results.params)
```

This tells us that that our best fit model - the one that minimises the sum of the squares of the residuals -  is $y = 0.63x + 78.3$.  In other words, our `Final` test score can be expected to take a value of 78.3 even with zero score in the `Yr3` input, and that every additional point scored in `Yr3`  will increase the `Final` score by 0.63.
...
One natural way to benchmark how good a job your model does of explaining the outcome is to compare it to a situation where you have no input and no model at all.  In this situation, all you have is your outcome values which can be considered a random variable with a mean and a variance.  In the case of our six datapoints, we have six values of `Final` with a mean of `r round(mean(py$d$Final), 2)`.  We can consider the horizontal line representing the mean of $y$ as our 'random model', and we can calculate the residuals around the mean.  This can be seen in Figure \@ref(fig:model-mean).

```{r model-mean, fig.cap = "Residuals of our six datapoints around their mean value", fig.align = "center", echo = FALSE}
ggplot2::ggplot(data = py$d, aes(x = Yr3, y = Final)) +
  ggplot2::geom_point() +
  ggplot2::geom_function(fun = function(x) mean(d$Final), color = "blue", linetype = "dashed") +
  ggplot2::annotate("text", x = 180, y = 120, label = TeX("$y = 123.17$"), colour = "blue") +
  ggplot2::geom_segment(aes(x = Yr3[1], y = round(mean(d$Final), 2), xend = Yr3[1], yend = Final[1]), color = "blue") +
  ggplot2::geom_segment(aes(x = Yr3[2], y = round(mean(d$Final), 2), xend = Yr3[2], yend = Final[2]), color = "blue") +
  ggplot2::geom_segment(aes(x = Yr3[3], y = round(mean(d$Final), 2), xend = Yr3[3], yend = Final[3]), color = "blue") +
  ggplot2::geom_segment(aes(x = Yr3[4], y = round(mean(d$Final), 2), xend = Yr3[4], yend = Final[4]), color = "blue") +
  ggplot2::geom_segment(aes(x = Yr3[5], y = round(mean(d$Final), 2), xend = Yr3[5], yend = Final[5]), color = "blue") +
  ggplot2::geom_segment(aes(x = Yr3[6], y = round(mean(d$Final), 2), xend = Yr3[6], yend = Final[6]), color = "blue")

```

Recall from our previous chapter the definition of the population variance of $y$, notated as $\mathrm{Var}(y)$.  Note that it is defined as the average of the squares of the residuals around the mean of$y$. Therefore $\mathrm{Var}(y)$ represents the mean sum of squares error of a random model.  This calculates in this case to `r round((5/6)*var(py$d$Final), 2)`.
...
ggplot2::ggplot(data = py$d, aes(x = Yr3, y = Final)) +
  ggplot2::geom_point() +
  ggplot2::geom_function(fun = function(x) mean(d$Final), color = "blue", linetype = "dashed") +
  ggplot2::annotate("text", x = 180, y = 110, label = TeX("$y = 123.17$"), colour = "blue") +
  ggplot2::geom_segment(aes(x = Yr3[1], y = round(mean(d$Final), 2), xend = Yr3[1], yend = Final[1]), color = "blue") +
  ggplot2::geom_segment(aes(x = Yr3[2], y = round(mean(d$Final), 2), xend = Yr3[2], yend = Final[2]), color = "blue") +
  ggplot2::geom_segment(aes(x = Yr3[3], y = round(mean(d$Final), 2), xend = Yr3[3], yend = Final[3]), color = "blue") +
  ggplot2::geom_segment(aes(x = Yr3[4], y = round(mean(d$Final), 2), xend = Yr3[4], yend = Final[4]), color = "blue") +
  ggplot2::geom_segment(aes(x = Yr3[5], y = round(mean(d$Final), 2), xend = Yr3[5], yend = Final[5]), color = "blue") +
  ggplot2::geom_segment(aes(x = Yr3[6], y = round(mean(d$Final), 2), xend = Yr3[6], yend = Final[6]), color = "blue") +
  ggplot2::geom_function(fun = function(x) {py$results$params[2]*x + py$results$params[1]}, colour = "red", 
                         linetype = "dashed") +
  ggplot2::annotate("text", x = 160, y = 155, label = TeX("$y = 0.63x + 78.3$"), colour = "red") +
  ggplot2::geom_segment(aes(x = Yr3[1], y = py$results$fittedvalues[1], xend = Yr3[1], yend = Final[1]), color = "red") +
  ggplot2::geom_segment(aes(x = Yr3[2], y = py$results$fittedvalues[2], xend = Yr3[2], yend = Final[2]), color = "red") +
  ggplot2::geom_segment(aes(x = Yr3[3], y = py$results$fittedvalues[3], xend = Yr3[3], yend = Final[3]), color = "red") +
  ggplot2::geom_segment(aes(x = Yr3[4], y = py$results$fittedvalues[4], xend = Yr3[4], yend = Final[4]), color = "red") +
  ggplot2::geom_segment(aes(x = Yr3[5], y = py$results$fittedvalues[5], xend = Yr3[5], yend = Final[5]), color = "red") +
  ggplot2::geom_segment(aes(x = Yr3[6], y = py$results$fittedvalues[6], xend = Yr3[6], yend = Final[6]), color = "red")

```

So we seem to have reduced the variance from the random model by fitting our new model.  If we average the square of our residuals for the fitted model, we obtain the mean sum of squares error of our fitted model, which calculates to `r round(sum(py$results$resid^2)/6, 2)`.

Therefore before we fit our model, we have an average error of `r round((5/6)*var(py$d$Final), 2)`, and after we fit it, we have an average error of `r round(sum(py$results$resid^2)/6, 2)`.  So we have reduced the average error of our model by `r round((5/6)*var(d$Final), 2) - round(sum(py$results$resid^2)/6, 2)` or, expressed as a proportion, by `r round((round((5/6)*var(d$Final), 2) - round(sum(py$results$resid^2)/6, 2))/((5/6)*var(d$Final)), 2)`.  In other words, we can say that our model explains `r  round(((5/6)*var(d$Final) - sum(py$results$resid^2)/6)/((5/6)*var(d$Final)), 2)` (or `r  round(((5/6)*var(d$Final) - sum(py$results$resid^2)/6)/((5/6)*var(d$Final)), 2) * 100`%) of the variance of our outcome.  

This metric is known as the $R^2$ of our model and is the primary metric used in measuring the fit of a linear regression model^[As a side note, in a simple regression model like this, where there is only one input variable, we have the simple identity $R^2 = r^2$, where $r$ is the correlation between the input and outcome (for our small set of six datapoints here, the correlation is `r round(cor(py$d$Yr3, py$d$Final), 3)`)]. 
...
In this case, our input is a vector of $p$ variables $(x_1, x_2, ..., x_p)$.  Extending the linear equation in Figure \@ref(linear-single), we seek to develop an equation of the form:
...
```{python}
model = sm.ols(data = ugtests, formula = "Final ~ Yr3 + Yr2 + Yr1")
results = model.fit()
print(results.params)
```
...
* The intercept of the model is `r round(py$results$params[1], 3)`.  This is the value that a student could be expected to score in their final exam even if they had scored zero in all previous exams.

* The `Yr3` coefficient is `r round(py$results$params[2], 3)`.  Assuming no change in other inputs, this is the increase in the Final exam score that could be expected from an extra point in the Year 3 score. 

* The `Yr2` coefficient is `r round(py$results$params[3], 3)`.  Assuming no change in other inputs, this is the increase in the Final exam score that could be expected from an extra point in the Year 2 score. 

* The `Yr1` coefficient is `r round(py$results$params[4], 3)`.  Assuming no change in other inputs, this is the increase in the Final exam score that could be expected from an extra point in the Year 1 score. 
...
The `summary()` method is a useful way to gather critical information in your model, including important statistics on your coefficients:
...
This summary consists of three tables, the middle table of which gives statistical information on the estimated coefficients of the model:

```{python}
coefs_summary = model_summary.tables[1]
print(coefs_summary)
```


The 95% confidence interval corresponds to approximately two Standard Errors above or below the estimated value, and can be seen in the columns `[0.025, 0.975]`.  For example, the 95% confidence interval for the coefficient of `Yr3` is [`r round(as.numeric(py$coefs_summary$data[[3]][6]), 3)`, `r round(as.numeric(py$coefs_summary$data[[3]][7]), 3)`]. For a given coefficient, if this confidence interval includes zero, you cannot reject the hypothesis that the variable has no relationship with the outcome.  Another indicator of this is the column `P > |t|`, which represents the *p-value* of the statistical hypothesis test.  If this value is less than a certain threshold (usually 0.05), you can conclude that this variable has a statistically significant relationship with the outcome. In this case, we can conclude that the examinations in Years 2 and 3 have a significant relationship with the Final examination score, but we cannot conclude this for Year 1.  Effectively, this means that we can drop `Yr1` from our model with no significant loss of fit.  In general, simpler models are easier to manage and interpret, so let's remove the non-significant variable now.

```{python}
newmodel = sm.ols(data = ugtests, formula = "Final ~ Yr3 + Yr2")
...
At this point we can further explore the overall summary of our model.  As you saw in the previous section, our model summary contains three tables.  The first table contains some overall information on our model, including its fit:

```{python}
## get summary of model
newmodel_summary = newresults.summary()

## show overall fit statistics
print(newmodel_summary.tables[0])
```

We can see from the $R^2$ value that our model explains a lot of the variance in the Final examination score.   We can also see a report on our model confidence - that is, how confident can we be that our model fits the outcome better than the alternative of a random model.  The F-statistic is an evaluation of the confidence of the entire model, with a high F-statistic indicating a strong likelihood that the model fits the outcome better than a random model.  More intuitively, perhaps, we also have the p-value for the F-statistic, notated as `Prob (F-statistic)`.  In this case it is extremely small, so we can conclude that our model has some explanatory/predictive power over and above that of a random model.

Be careful not to confuse model confidence with $R^2$.  Depending on your sample, it is entirely possible for a model with a low $R^2$ to have high confidence and vice versa.  

### Making predictions from your model

In this case we should already be quite happy with the overall fit and confidence level of our linear regression model, and we may now want to use it to predict the Final test scores of our new students who have been affected by the pandemic. While the book focuses on inferential rather than predictive analytics, we briefly touch here on the mechanics of generating predictions from models. As you might imagine, once the model has been fitted, prediction is a relatively straightforward process.  We feed in the `Yr2` and `Yr3` examination scores into our fitted model, and it applies the coefficients to calculate the predicted outcome.  Let's look at three fictitious students, and create a dataframe with their scores to input into the model.

```{python}
Yr2 = [67, 23, 88] 
Yr3 = [144, 100, 166]

new_students = dict(Yr2 = Yr2, Yr3 = Yr3)
new_students = pd.DataFrame(new_students)
```

Now we can feed these values into our fitted model to get predictions of the Final examination result for our three new students.

```{python}
## use newmodel to predict for new_students
predictions = newresults.get_prediction(new_students)
...
1.  If the data for a given variable is relatively complete and only a small number of observations are missing, its usually best and simplest to remove the observations that are missing from the dataset.  Note that many modeling functions (though not all) will take care of this automatically.
...
```{python}
make = ["Ford", "Toyota", "Audi"] 
manufacturing_cost = [15000, 19000, 28000]

vehicle_data = dict(make = make, manufacturing_cost = manufacturing_cost)
...
```{python}
dummy_vehicle = pd.get_dummies(vehicle_data['make'])
print(dummy_vehicle)
```
...
```{python}
vehicle_data_dummies = pd.concat([vehicle_data.drop("make", axis = 1), dummy_vehicle], axis = 1)
print(vehicle_data_dummies)
```
...
## Important watchouts in linear regression

All modeling techniques have underlying assumptions about the data that they model, and can generate inaccurate results when those assumptions do not hold true.  Conscientious analysts will verify that these assumptions are satisfied before finalizing their modeling efforts.  In this section we will outline some common 'watchouts' when running linear regression models.

### Assumption of linearity and additivity

We mentioned earlier that linear regression assumes that the relationship we are trying to model is linear and additive in nature.  Therefore you can expect problems if you are using this approach to model a pattern that is non-linear.  In general, you'd expect this problem to come out 'in the wash' anyway, because a linear model will not produce a good fit on a non-linear underlying relationship.  

Nevertheless, you can check whether your linearity assumption was reasonable in a couple of ways.  You can plot the true versus the predicted (fitted) values to see if they look correlated.  You can see such a plot on our student examination model from before in Figure \@ref(fig:predplot):

```{python predplot, fig.cap = "Plot of true versus fitted/predicted student scores", fig.align = "center"}
predicted_values = newresults.fittedvalues
true_values = ugtests["Final"]

#plot true values against predicted values
predplot = plt.scatter(predicted_values, true_values)
plt.show(predplot)
...
```{python resplot, fig.cap = "Plot of residuals against fitted/predicted scores", fig.align = "center",}
residuals = newresults.resid

#plot residuals against predicted values
resplot = plt.scatter(predicted_values, residuals)
plt.show(resplot)
```
...
```{python resyr2, fig.cap = "Plot of residuals against `Yr2` values", fig.align = "center"}
Yr2 = ugtests["Yr2"]

#plot residuals against Yr2 values
resYr2plot = plt.scatter(Yr2, residuals)
plt.show(resYr2plot)
```

Then we can plot against `Yr3` in Figure \@ref(fig:resyr3):

```{python resyr3, fig.cap = "Plot of residuals against `Yr3` values", fig.align = "center"}
Yr3 = ugtests["Yr3"]

#plot residuals against Yr3 values
resYr3plot = plt.scatter(Yr3, residuals)
plt.show(resYr3plot)
```

Both plots show a pretty consistent range of values between the left and right sides of the plots which reassures us that we have homoscedacity.
...
```{python qqplottests, fig.cap = "Quantile-Quantile plot of residuals", fig.align = "center"}
...
## Extending multiple linear regression into non-linear contexts
...
Recall that our model of student examination scores took each year's score as a separate input variable, and therefore we are making the assumption that the score obtained in each years acts independently and additively in predicting the Final score.  However, it is very possible that several input variables act together in relation to the outcome.  One way of modelling this is to include interaction terms in your model, which are new input variables formed as products of the original input variables.
...
```{python}
interaction_model = sm.ols(data = ugtests, formula = "Final ~ Yr2 + Yr3 + Yr2*Yr3")
interaction_results = interaction_model.fit()
print(interaction_results.summary())
```

We see that introducing this interaction term has improved our model, and that the interaction term is significant, so we conclude that in addition to a significant effect of the `Yr2` an `Yr3` scores, there is an additional significant effect from their interaction.  

In theory, an analyst could explore any combination of interactions of two or more variables.  However, consider the impact on interpretability of modeling complex combinations of interactions.  As always, there is a trade-off between intepretability and accuracy.  In the case of the model above, now with a near perfect fit, there would likely be no further benefit to exploring additional interaction terms, if those did exist.
...
Specific non linear models can be trialled using linear regression technology.  For example, if the underlying relationship was throught to be quadratic on a given input variable $x$, then the formula would take the form $y = \beta_0 + \beta_1x + \beta_2x^2$, and this can easily be put into a linear model formula.  
...
```{python}
quadratic_yr1_model = sm.ols(data = ugtests, formula = "Final ~ Yr3 + Yr2 + Yr1 + I(Yr1^2)")
quadratic_yr1_results = quadratic_yr1_model.fit()
print(quadratic_yr1_results.summary())
```
...
Most chapters in this book will end with learning exercises.  These will be a combination of discussion questions to encourage engagement with the material in the chapter and practical exercises with data sets.  Links to data will be provided where relevant.
