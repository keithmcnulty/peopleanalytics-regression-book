...
## What it is and when to use it {#when-oßls}
...
# use the readr library to directly download our data into a dataframe called ugtests
library(readr)
ugtests <- readr::read_csv("https://raw.githubusercontent.com/keithmcnulty/eampa/master/data/ugtests.csv")
...
```{r, echo = FALSE}
head(ugtests[ , c("Yr3", "Final")])
```

We can do a simple plot of these data points as in \@ref(fig:sample-plot):
...
Intuitively, we can imagine a line passing through these points that 'fits' the general pattern.  For example, taking $m = 2$ and $c = 0$, the resulting line $y = 2x$ could  fit between the points we are given, as displayed in \@ref(fig:sample-plot-fit):

``` {r sample-plot-fit, echo = FALSE}
ggplot2::ggplot(data = d, aes(x = Yr3, y = Final)) + 
  ggplot2::geom_point() +
...
Now let's overlay our fitted model onto this random model in \@ref(fig:model-overlay):

```{r model-overlay, echo = FALSE}
ggplot2::ggplot(data = d, aes(x = Yr3, y = Final)) +
  ggplot2::geom_point() +
  ggplot2::geom_function(fun = function(x) mean(d$Final), color = "blue", linetype = "dashed") +
...
In this case, our input $X$ is a vector of $p$ variables $(x_1, x_2, ..., x_p)$.  Extending the linear equation in \@ref(linear-single), we seek to develop an equation of the form:
...
Referring to our formula in \@ref(multiple-linear-regression), let's understand what each coefficient $\beta_0, \beta_1, ..., \beta_p$ means.  $\beta_0$, known as the *intercept* of the model, represents the value of $y$ assuming that all the inputs were zero.  You can imagine that your output can be expected to have a base value even without any inputs - a student who completely flunked the first three years can still redeem themselves to a degree in the Final year.
...
3.  Moderate sparse data could be considered for imputation.  Imputation methods involve using the overall statistical properties of the entire data set or of specific other variables to 'suggest' what the missing value might be.  We will not cover imputation methods in depth here, but we will touch on them later in this book. 
...
Nevertheless, you can check whether your linearity assumption was reasonable in a couple of ways.  You can plot the true versus the predicted (fitted) values to see if they look correlated.  You can see such a plot on our student examination model from before in \@ref(fig:predplot):
...
Alternatively, you can plot the residuals of your model against the predicted values, and look for the pattern of a random distribution (ie no major discernible pattern), such as in \@ref(fig:resplot):
...
While a simple plot of residuals against predicted values (such as in \@ref(fig:resplot)) can give a quick indication on homoscedacity, to be thorough the residuals should be plotted against each input variable, and it should be verified that the range of the residuals remains broadly stable.  In our student examination model, we can first plot the residuals against the values of `Yr2` in \@ref(fig:resyr2):
...
Then we can plot against `Yr3` in \@ref(fig:resyr3):
